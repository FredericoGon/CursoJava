...
public class LoginBean {
	...
	public String autenticar(){
		...
		if("b@teste.com".equals(email) && "e10adc3949ba59abbe56e057f20f883e".equals(gerarHash(senha))){
			Usuario usuario = new Usuario("Barginho", email, senha);
			HttpSession session = (HttpSession)ctx.getExternalContext().getSession(true);
			session.setAttribute("usuarioLogado", usuario);
			return "index.xhtml?faces-redirect=true";
		}else{
			FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_ERROR, "Login Inválido", "Email inválido");	
	...
	public static void main(String[] args) {
		System.out.println(gerarHash("123456"));
	}
