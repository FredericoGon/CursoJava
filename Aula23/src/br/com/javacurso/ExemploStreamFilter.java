...
public class ExemploStreamFilter {
	...
	public static void main(String[] args) {
		...
		produtos.add(new Produto(6, "Produto 06", 19.9, Categoria.ALIMENTO));
		...
		produtos.add(new Produto(10, "Produto 10", 89.9, Categoria.VESTUARIO));
		
		Stream<Produto> streamProdutos = produtos.stream();
		streamProdutos
		.filter((Produto produto)->{return Categoria.ALIMENTO.equals(produto.getCategoria());})
		.map(p-> p.getNome())
		...
		Stream<Produto> streamPerfume = produtos.stream();
		streamPerfume
		.filter((Produto produto)->{return Categoria.PERFUMARIA.equals(produto.getCategoria());})
		.map(p-> p.getNome())
		.forEach(System.out::println);
		
		System.out.println("===================================================");
		
		List<Produto> streamBaratos = produtos
		.stream()
		.filter((Produto produto)->{
			return produto.getPreco()<=50.00;
			})
		.collect(Collectors.toList());
		
		for (Produto prod : streamBaratos) {
			System.out.println(prod.getNome());
		}
		
		System.out.println("===================================================");
		
		List<String> nomesMaioresQue50 = produtos
				.stream()
				.filter(p-> p.getPreco()>50)
				.map(p->p.getNome())
				.collect(Collectors.toList());
		for (String nomesProdutos : nomesMaioresQue50) {
		...
		Map<Categoria, Double> totalizador = produtos
				...
				.reduce(map, (current, prod)->{
					Double value = map.get(prod.getCategoria());
					if(value == null){
						value = 0.0;
					}
					value += prod.getPreco();
					map.put(prod.getCategoria(), value);
					return map;
				}, (i,j)->{
					return i;
				});
		...
		System.out.println("===================================================");
		Map<Categoria, Double> map2 = new HashMap<>();
		Map<Categoria, Double> totalizador2 = produtos
				.stream()
				.filter(p->p.getPreco()<50)
				.reduce(map2,
						(current, prod)->{
		...
		for (Map.Entry<Categoria, Double> item : totalizador2.entrySet()) {
			System.out.printf("%s = %.2f\n",item.getKey(), item.getValue());
		}
		System.out.println("===================================================");		
		...
		System.out.println("O produo de menor preço é: "+produtoMenor.get().getNome());
